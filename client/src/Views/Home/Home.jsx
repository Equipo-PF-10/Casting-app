...
const Home = (props) => {
  ...
  useEffect(() => {
    if(props.type === "talent") {
       axios(`https://deploy-sprint-2-backend.onrender.com/talents/${localStorage.getItem("user_id")}`).then(({ data }) => {
        setUser(data);
      });
    } else {
       axios(`https://deploy-sprint-2-backend.onrender.com/companies/${localStorage.getItem("user_id")}`).then(({ data }) => {
        setUser(data);
      });
    }
