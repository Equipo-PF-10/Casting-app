...
const Detail = (props) => {
  ...
  const handlerClickCreate = async () => {
    ...
    if(talent.id === "" || talent.image === null || talent.hability === null){
      ...
      const { CompanyId } = (await (axios.get(`https://casting-app-thdg.onrender.com/events/eventid/${idEvent}`))).data
      
      const { email } = (await (axios.get(`https://casting-app-thdg.onrender.com/companies/${CompanyId}`))).data     

      const emailToCompany = axios.post(`https://casting-app-thdg.onrender.com/email/newPostulante/${email}`)
                .then((resp) => console.log(resp.data))
                .catch((error) => console.log(error));
      
      const talentEmail = (await (axios.get(`https://casting-app-thdg.onrender.com/talents/${idTalent}`))).data.email          
      
      const emailToTalent = axios.post(`https://casting-app-thdg.onrender.com/email/postulationEvent/${talentEmail}`) 
                .then((resp) => console.log(resp.data))
                .catch((error) => console.log(error));
    }   
  }
