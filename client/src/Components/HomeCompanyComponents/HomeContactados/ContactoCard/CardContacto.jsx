...
const CardContacto = (props) => {
  ...
  const handleClickContratarConfirmation = async () => {
    ...
    try {
      const talentResponse = await axios.get(`https://deploy-sprint-2-backend.onrender.com/talents/${props.id_talent}`);
      const talentEmail = talentResponse.data.email;
  
      const emailToTalent = await axios.post(`https://deploy-sprint-2-backend.onrender.com/email/talenContacHired/${talentEmail}`);
      console.log(emailToTalent.data);
  
      const emailToCompany = axios.post(`https://deploy-sprint-2-backend.onrender.com/email/talenContacHiredForCompany/${localStorage.getItem("user_email")}`)
  ...
  const handleClickRechazarConfirmation = async () => {
    ...
    try {
      ...
      const emailToTalent = await axios.post(`https://deploy-sprint-2-backend.onrender.com/email/talentContacRefused/${talentEmail}`);
      console.log(emailToTalent.data);
  
      const emailToCompany = axios.post(`https://deploy-sprint-2-backend.onrender.com/email/talentContacRefusedForCompany/${localStorage.getItem("user_email")}`)
